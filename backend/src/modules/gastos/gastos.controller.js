...
const resumenMensual = async (req, res, next) => {
  try {
    const resumen = await service.resumenMensual(req.user.id, req.validatedQuery || req.query);
...
const resumenCategoria = async (req, res, next) => {
  try {
    const resumen = await service.resumenCategoria(req.user.id, req.validatedQuery || req.query);

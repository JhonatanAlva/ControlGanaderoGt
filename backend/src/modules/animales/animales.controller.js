...
const listar = async (req, res, next) => {
  try {
    ...
    paginated(res, { data, total, page, limit, message: 'Animales obtenidos.' });
  } catch (err) { next(err); }
};
...
const crear = async (req, res, next) => {
  try {
    const animal = await service.crear(req.user.id, req.body, req.user.plan, req.file);
...
const actualizar = async (req, res, next) => {
  try {
    const animal = await service.actualizar(req.params.id, req.user.id, req.body, req.file);
...
const cambiarEstado = async (req, res, next) => {
  try {
    const animal = await service.cambiarEstado(req.params.id, req.user.id, req.body);
...
const registrarPeso = async (req, res, next) => {
  try {
    const peso = await service.registrarPeso(req.params.id, req.user.id, req.body);
...
module.exports = {
  ...
  cambiarEstado, eliminar, registrarPeso, historialPesos,
};